...
from gpax import ExactGP, RBFKernel
...
class HeteroscedasticNoise(Noise):
    ...
    def __call__(self, params, X):
        noise_gp = ExactGP(kernel=RBFKernel(active_dims=list(range(X.shape[1]))))
        ...
        covar = B.dense(noise_gp.kernel(params["noise_gp"])(X_inducing, X_inducing))
        ...
        return jnp.exp(
            noise_gp.predict(params["noise_gp"], X_inducing, latent_log_variance, X, return_cov=False)
    ...
    def __initialise_params__(self, key, X_inducing):
        ...
        noise_gp = ExactGP(kernel=RBFKernel(active_dims=list(range(X_inducing.shape[1]))))

        params["noise_gp"] = noise_gp.initialise_params(key, X_inducing)
    ...
    def __get_bijectors__(self):
        noise_gp = ExactGP()
        bijectors = {"noise_gp": noise_gp.get_bijectors(), "inducing_std_noise": tfb.Identity()}
        if self.X_inducing is not None:
